  ...
  <div class="color-whist-game">
    ...
    <div v-else-if="game" class="game-container">
      <div class="game-header">
        ...
        <h1>{{ t('colorWhistGame.title') }} - {{ t('round') }} {{ game.current_round + 1 }}</h1>
      ...
      <div v-if="hasActiveContract && previewScores.length > 0" class="score-preview card">
        <div class="preview-header">
          ...
          <strong>{{ t('colorWhistGame.scorePreview') }}</strong>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="contract-col">{{ t('colorWhistGame.contract') }}</th>
          ...
          <tbody>
            ...
            <!-- Totals row -->
            <tr class="totals-row">
              <td class="round-col"><strong>{{ t('colorWhistGame.total') }}</strong></td>
              ...
              <td v-for="(player, index) in game.players" :key="player.id" class="input-cell" :class="getCurrentTeamClass(index)">
                <div class="color-whist-input">
                  ...
                  <input
                    ...
                    :placeholder="t('colorWhistGame.opponentTricks')"
                  />
                  ...
                  <input
                    ...
                    :placeholder="t('colorWhistGame.tricksPlaceholder')"
      ...
      <div class="contract-legend card">
        <h3>{{ t('colorWhistGame.contractLegend') }}</h3>
        <div class="legend-grid">
          <div class="legend-section">
            <h4 class="legend-title team">{{ t('colorWhistGame.legendTeam') }}</h4>
            <div class="legend-items">
              <span class="legend-item">Accept 8-13, Trull</span>
            </div>
          </div>
          <div class="legend-section">
            <h4 class="legend-title solo">{{ t('colorWhistGame.legendSolo') }}</h4>
            <div class="legend-items">
              <span class="legend-item">Solo 5-8, Abondance, Piccolo, Grand Slam</span>
            </div>
          </div>
          <div class="legend-section">
            <h4 class="legend-title misery">{{ t('colorWhistGame.legendMisery') }}</h4>
            <div class="legend-items">
...
import type {
  Game,
  ColorWhistContract,
  ColorWhistRoundRequest,
  ColorWhistSelectionPayload,
} from '../types'

type TrickValue = number | null
type ContractKey = 'none' | ColorWhistContract
...
const contractOptions: ContractOption[] = [
  { key: 'none', labelKey: 'colorWhistGame.contracts.none' },
  { key: 'accept_8', labelKey: 'colorWhistGame.contracts.accept8' },
  { key: 'accept_9', labelKey: 'colorWhistGame.contracts.accept9' },
  { key: 'accept_10', labelKey: 'colorWhistGame.contracts.accept10' },
  { key: 'accept_11', labelKey: 'colorWhistGame.contracts.accept11' },
  { key: 'accept_12', labelKey: 'colorWhistGame.contracts.accept12' },
  { key: 'accept_13', labelKey: 'colorWhistGame.contracts.accept13' },
  { key: 'solo_5', labelKey: 'colorWhistGame.contracts.solo5' },
  { key: 'solo_6', labelKey: 'colorWhistGame.contracts.solo6' },
  { key: 'solo_7', labelKey: 'colorWhistGame.contracts.solo7' },
  { key: 'solo_8', labelKey: 'colorWhistGame.contracts.solo8' },
  { key: 'abondance_9', labelKey: 'colorWhistGame.contracts.abondance9' },
  { key: 'abondance_10', labelKey: 'colorWhistGame.contracts.abondance10' },
  { key: 'abondance_11', labelKey: 'colorWhistGame.contracts.abondance11' },
  { key: 'abondance_12', labelKey: 'colorWhistGame.contracts.abondance12' },
  { key: 'trull', labelKey: 'colorWhistGame.contracts.trull' },
  { key: 'piccolo', labelKey: 'colorWhistGame.contracts.piccolo' },
  { key: 'small_misery', labelKey: 'colorWhistGame.contracts.smallMisery' },
  { key: 'large_misery', labelKey: 'colorWhistGame.contracts.largeMisery' },
  { key: 'open_misery', labelKey: 'colorWhistGame.contracts.openMisery' },
  { key: 'grand_slam', labelKey: 'colorWhistGame.contracts.grandSlam' },
]
...
const getContractDisplay = (roundIndex: number): string => {
  ...
  if (nonZero.length === 0) return t('colorWhistGame.contracts.none') as string
  ...
  if (positive.length + negative.length <= 4) {
    return t('colorWhistGame.legendMisery') as string
  }
...
const loadGame = async () => {
  try {
    ...
    if (data.players.length !== 4) {
      throw new Error(t('colorWhistGame.errors.playerCount') as string)
    }
    game.value = data
...
const ensureTrickValue = (playerIndex: number, contractKey: ContractKey): number => {
  ...
  if (raw === null || Number.isNaN(raw)) {
    throw new Error(t('colorWhistGame.errors.tricksRequired', {
      player: game.value!.players[playerIndex].name,
      contract: getContractLabel(contractKey),
    }) as string)
  }
  if (raw < 0 || raw > 13) {
    throw new Error(t('colorWhistGame.errors.tricksRange', {
      player: game.value!.players[playerIndex].name,
    }) as string)
  }
...
const ensureTeamTricks = (participants: number[], contractKey: ContractKey) => {
  ...
  for (let i = 1; i < participants.length; i++) {
    ...
    if (value !== base) {
      throw new Error(t('colorWhistGame.errors.teamSync', {
        contract: getContractLabel(contractKey),
      }) as string)
    }
  }
...
const buildColorWhistRound = (): ColorWhistRoundRequest => {
  if (!game.value) {
    throw new Error(t('colorWhistGame.errors.missingGame') as string)
  }

  const selections: ColorWhistSelectionPayload[] = []
  ...
  if (primaryGroups.size > 1) {
    throw new Error(t('colorWhistGame.errors.multiplePrimary') as string)
  }
  ...
  if (primaryEntry && miseryPlayers.length > 0) {
    throw new Error(t('colorWhistGame.errors.miseryConflict') as string)
  }

  if (!primaryEntry && miseryPlayers.length === 0) {
    throw new Error(t('colorWhistGame.errors.missingContract') as string)
  }

  if (primaryEntry) {
    const [key, participants] = primaryEntry
    const contract = key as ColorWhistContract

    if (isTeamContract(key)) {
      if (participants.length !== 2) {
        throw new Error(t('colorWhistGame.errors.teamCount') as string)
      }
      ...
      if (participants.length !== 1) {
        throw new Error(t('colorWhistGame.errors.soloCount') as string)
      }
      ...
      throw new Error(t('colorWhistGame.errors.unsupportedContract') as string)
    }
  }
...
const submitRound = async () => {
  ...
  let colorWhistRound: ColorWhistRoundRequest
  try {
    colorWhistRound = buildColorWhistRound()
  } catch (err: any) {
    scoreError.value = err?.message || (t('colorWhistGame.errors.validation') as string)
    return
  }

  try {
    const updated = await gamesApi.addRound(gameId, { colorWhistRound })
    game.value = updated
    resetSelections(updated.players.length)
  } catch (err: any) {
    scoreError.value = err.response?.data?.error || (t('colorWhistGame.errors.submitFailed') as string)
  }
}
...
.color-whist-input select,
.color-whist-input input {
  width: 100%;
  padding: 8px;
  border-radius: 8px;
...
.color-whist-input select.team-selected {
  border-color: var(--accent-emerald);
  background-color: rgba(47, 92, 76, 0.08);
}

.color-whist-input select.solo-selected {
  border-color: #e67e22;
  background-color: rgba(230, 126, 34, 0.08);
}

.color-whist-input select.misery-selected {
  border-color: #9b59b6;
  background-color: rgba(155, 89, 182, 0.08);
}

.color-whist-input input.synced {
  border-color: var(--accent-emerald);
}

.color-whist-input input.auto-calculated {
...
.color-whist-input select.opponent-auto {
  opacity: 0.6;
}
