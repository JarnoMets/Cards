<template>
  <div class="page-container color-whist-setup">
    <div class="content-container setup-container">
      <SetupHeader 
        :title="t('colorWhistGame.title')" 
        @back="goToGameSelection" 
      />

      <!-- Game type selection (contracts) -->
      <div class="info-section card">
        <h1 class="scribble-underline">{{ t('colorWhistRules.about') }}</h1>
        <p>{{ t('colorWhistRules.contractSelectionHint') }}</p>
        ...
        <div class="game-settings">
          <h3>{{ t('colorWhistSetup.settings') }}</h3>
          <label class="checkbox-row setting-row">
            <input
              type="checkbox"
              v-model="passRoundDoubling"
            />
            <span>{{ t('colorWhistSetup.passRoundDoubling') }}</span>
          </label>
          <p class="setting-hint">{{ t('colorWhistSetup.passRoundDoublingHint') }}</p>
        </div>
      </div>
...
const { t } = useI18n()
const gameConfig = getGameConfig('color_whist')

// Contract keys; these will be sent to the game page via route query later if needed
const contracts = [
  { key: 'accept', labelKey: 'colorWhistGame.contracts.accept' },
  { key: 'solo', labelKey: 'colorWhistGame.contracts.solo' },
  { key: 'smallMisery', labelKey: 'colorWhistGame.contracts.smallMisery' },
  { key: 'piccolo', labelKey: 'colorWhistGame.contracts.piccolo' },
  { key: 'abondance', labelKey: 'colorWhistGame.contracts.abondance' },
  { key: 'trull', labelKey: 'colorWhistGame.contracts.trull' },
  { key: 'largeMisery', labelKey: 'colorWhistGame.contracts.largeMisery' },
  { key: 'openMisery', labelKey: 'colorWhistGame.contracts.openMisery' },
  { key: 'grandSlam', labelKey: 'colorWhistGame.contracts.grandSlam' },
]
